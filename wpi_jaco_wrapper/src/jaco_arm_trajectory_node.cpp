    ...
    : arm_initialized (false)
{
    ...
    setForceControlServer = nh.advertiseService(topic_prefix_+"_arm/SetForce", &JacoArmTrajectoryController::setForceCallback, this);
    configCartesianInertiaDampingServer = nh.advertiseService(topic_prefix_+"_arm/config_InertiaDamping",
                                          &JacoArmTrajectoryController::configCartesianInertiaDampingCallback,this);
...
void JacoArmTrajectoryController::execute_gripper(const control_msgs::GripperCommandGoalConstPtr &goal)
{
    ...
    while (gripperMoving)
    {
        ...
        if (gripper_server_->isPreemptRequested() || !ros::ok())
        {
            ...
            //preempt action server
            ROS_INFO("Gripper action server preempted by getAngularPositionclient");
...
bool JacoArmTrajectoryController::setForceCallback(wpi_jaco_msgs::SetForceControl::Request &req, wpi_jaco_msgs::SetForceControl::Response &res)
{
    int saved_controlType;

    if (req.forceControl == true)
    {
        {
            boost::recursive_mutex::scoped_lock lock(api_mutex);
            GetControlType(saved_controlType);
            ROS_INFO("Control Type is %d\n",saved_controlType);
        }
        if (saved_controlType == 1)
            ROS_INFO("Setting Cartesian Control");
        {
            boost::recursive_mutex::scoped_lock lock(api_mutex);
            SetCartesianControl();
        }
        ROS_INFO("Setting Force Control ON");
        {
            boost::recursive_mutex::scoped_lock lock(api_mutex);
            res.status = StartForceControl();
        }
    ...
    else
    {
        ...
        ROS_INFO("Setting back to Angular Control");
        {
            boost::recursive_mutex::scoped_lock lock(api_mutex);
            SetAngularControl();
        }

        ROS_INFO("Stopping Force Control");
        {
            boost::recursive_mutex::scoped_lock lock(api_mutex);
            res.status = StopForceControl();
        }
    }
    return true;
}
bool JacoArmTrajectoryController::configCartesianInertiaDampingCallback(wpi_jaco_msgs::ConfigCartesianInertiaDamping::Request &req,
                                                                        wpi_jaco_msgs::ConfigCartesianInertiaDamping::Response &res)
{
    CartesianInfo Inertia, Damping;

    Inertia.X = req.inertia.linear.x;
    Inertia.Y = req.inertia.linear.y;
    Inertia.Z = req.inertia.linear.z;

    Inertia.ThetaX = req.inertia.angular.x;
    Inertia.ThetaY = req.inertia.angular.y;
    Inertia.ThetaZ = req.inertia.angular.z;
    Damping.X = req.damping.linear.x;
    Damping.Y = req.damping.linear.y;
    Damping.Z = req.damping.linear.z;

    Damping.ThetaX = req.damping.angular.x;
    Damping.ThetaY = req.damping.angular.y;
    Damping.ThetaZ = req.damping.angular.z;
    {

        boost::recursive_mutex::scoped_lock lock(api_mutex);
        int result = SetCartesianInertiaDamping(Inertia, Damping);
    }
    // !! I don't know how to pass the node handle through the advertise service call. Probably using the node handle here is wrong. So I'll
    // need th change this later.
    ros::NodeHandle n;
    // Update Parameter server
    n.setParam("/kinova_forceControl/Inertia/X",req.inertia.linear.x);
    n.setParam("/kinova_forceControl/Inertia/Y",req.inertia.linear.y);
    n.setParam("/kinova_forceControl/Inertia/Z",req.inertia.linear.z);
    n.setParam("/kinova_forceControl/Inertia/ThetaX",req.inertia.angular.x);
    n.setParam("/kinova_forceControl/Inertia/ThetaY",req.inertia.angular.y);
    n.setParam("/kinova_forceControl/Inertia/ThetaZ",req.inertia.angular.z);

    n.setParam("/kinova_forceControl/Damping/X",req.damping.linear.x);
    n.setParam("/kinova_forceControl/Damping/Y",req.damping.linear.y);
    n.setParam("/kinova_forceControl/Damping/Z",req.damping.linear.z);
    n.setParam("/kinova_forceControl/Damping/ThetaX",req.damping.angular.x);
    n.setParam("/kinova_forceControl/Damping/ThetaY",req.damping.angular.y);
    n.setParam("/kinova_forceControl/Damping/ThetaZ",req.damping.angular.z);
    res.status = true;

    ROS_INFO("Setting Inertia and Damping Parameters");
    return true;
}
